...
type
  TDm = class(TDataModule)
    ...
    FDQUsuarioFOTO: TWideStringField;
  ...
  function ExitUsuarioCadastrado : Boolean;
...
begin
  ...
  begin
    ...
    Params.Clear;
    Params.Values['DriverID']     := 'SQLite';
    Params.Values['Database']     :=  TPath.Combine(TPath.GetDocumentsPath, 'financeiro.db');
    ...
    {$IFDEF MSWINDOWS}
    try
      Params.Values['Database'] := 'D:\AppFinanceiro99\trunk\db\financeiro.db';
      Connected := true;
...
procedure TDm.delete;
begin
  FDQuery1.Close;
  FDQuery1.SQL.Clear;
  FDQuery1.SQL.Add('PRAGMA foreign_keys = off;'+
                    'BEGIN TRANSACTION;'       +

'DELETE FROM TAB_CATEGORIA;'                   +
'DELETE FROM TAB_CONFIG;'                       +
'DELETE FROM TAB_LANCAMENTO;'                    +
...
function TDm.ExitUsuarioCadastrado: Boolean;
begin
   FDQUsuario.Close;
   FDQUsuario.Open();
...
begin
  ...
  begin
    ...
    If FDQUsuario.RecordCount > 0 then
    begin
      while FDQUsuario.RecordCount <> 0 do
        Delete;
      Close;
      Open();
    end;
